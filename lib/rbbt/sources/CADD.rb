...
module CADD
  ...
  CADD.claim CADD['.source'][File.basename(URL).sub('.gz','')], :url, URL
  ...
  def self.database
    @@database ||= begin
                     Persist.persist_tsv("CADD", CADD['.source'][File.basename(URL).sub('.gz','')].find, {}, :persist => true,
                       ...
                       file = CADD['.source'][File.basename(URL).sub('.gz','')]
                       last_chr = nil
                       last_pos = nil
                       last_values = {}
